...
def get_processed_videos() -> set:
    """Load the set of already-processed video IDs."""
    if config.PROCESSED_VIDEOS_FILE.exists():
        return set(config.PROCESSED_VIDEOS_FILE.read_text().strip().splitlines())
    return set()


def mark_processed(video_id: str):
    """Mark a video ID as processed."""
    with open(config.PROCESSED_VIDEOS_FILE, "a") as f:
        f.write(video_id + "\n")
...
def download_video(video_url: str, video_id: str) -> Path | None:
    """Download a video and return the path to the downloaded file."""
    output_path = config.VIDEOS_DIR / f"{video_id}.%(ext)s"
    ...
    try:
        ...
        for f in config.VIDEOS_DIR.glob(f"{video_id}.*"):
